	...
	{ 
		...
		throws Exception
		{
				...
				FasterWriter out = new FasterWriter(output);	
 				...
 				TWINRO obj = new TWINRO();

 				for(int i=1;i<=N;i++)
 				{
 					for(int j=1;j<=N;j++)
 					{
 						obj.board[i][j] = in.nextInt();
 					}
 				}
...
{                    	if(i==1)
                    		move[i][j] = move[i][j-1] + board[i][j] + board[j][N-i+1];
                    	
                    	else if(j==1)
                    		move[i][j] = move[i-1][j] + board[i][j] + board[j][N-i+1];
                    	
                    	else
                    		move[i][j] = Math.max(move[i][j-1],move[i-1][j]) + board[i][j] + board[j][N-i+1];
                	}
            	}
